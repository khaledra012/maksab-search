...
const DIRECT_GOOGLE_MAPS_API_KEY =
  import.meta.env.VITE_GOOGLE_MAPS_API_KEY?.trim();
const LEGACY_FORGE_MAPS_API_KEY =
...
function loadMapScript() {
  const scriptUrl = DIRECT_GOOGLE_MAPS_API_KEY
    ? `https://maps.googleapis.com/maps/api/js?key=${DIRECT_GOOGLE_MAPS_API_KEY}&v=weekly&libraries=marker,places,geocoding,geometry`
    : LEGACY_FORGE_MAPS_API_KEY
      ? `${LEGACY_FORGE_BASE_URL.replace(/\/+$/, "")}/v1/maps/proxy/maps/api/js?key=${LEGACY_FORGE_MAPS_API_KEY}&v=weekly&libraries=marker,places,geocoding,geometry`
      : null;

  if (!scriptUrl) {
    console.error(
      "Google Maps is not configured. Set VITE_GOOGLE_MAPS_API_KEY to load the map."
    );
